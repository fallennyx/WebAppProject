...
function sendChat() {
    ...
    const xsrfTokenElement = document.querySelector("input[name='xsrf_token']");
    const xsrfToken = xsrfTokenElement ? xsrfTokenElement.value : "";
    ...
    if (ws) {
        ...
        // Include the XSRF token in the payload, even if it's empty
        const messageJSON = { "message": message, "xsrf_token": xsrfToken };
        request.open("POST", "/chat-messages", true);
